...
SELECT CASE WHEN jobTitle = "President" OR jobTitle = "VP Sales" OR jobTitle = "VP Marketing" THEN "Executive"
...
# STEP 8
# Replace None with your code
sum_total_price = pd.read_sql("""SELECT ROUND(quantityOrdered * priceEach) AS total_price FROM orderDetails""", conn).sum()

# STEP 9
# Replace None with your code
df_day_month_year = pd.read_sql('''
SELECT orderDate, strftime('%d', orderDate) AS day, strftime('%m', orderDate) AS month, strftime('%Y', orderDate) AS year 
                                FROM Orders
                                ''',conn)
